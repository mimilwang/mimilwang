## Mimi Wang

# Hi, I'm Mimi Wang

**Computer Science @ University of Washington Seattle '27**  
*Business & Entrepreneurship Minor*

Building AI-powered products that solve real problems.

Currently developing an AI Study Copilot that helps students learn from lectures, assignments, and course materials using retrieval-augmented generation (RAG), vector search, and modern web technologies.

📍 Seattle, WA

---

## 🚀 What I'm Building

### Atlas -- AI Study Copilot

An AI-powered learning platform that transforms course materials into personalized study experiences.

**Current Focus**
- File uploads and document processing
- Retrieval-Augmented Generation (RAG)
- Vector databases
- AI-powered tutoring
- Authentication and user accounts
- Full-stack product development

---

## 🛠 Featured Projects

### wAI Travel Planner
**React · AWS · Gemini · DynamoDB · Mapbox**

🥈 2nd Place at DubHacks

Built an AI travel planning platform featuring personalized itinerary generation, drag-and-drop scheduling, user accounts, and location-aware recommendations.

**Highlights**
- AI-generated itineraries
- Interactive travel scheduling
- User authentication
- Saved travel preferences
- Multi-city trip planning

---

### VolunQueer
**Django · React · Docker**

Volunteer coordination platform designed to connect LGBTQ+ community members with meaningful volunteer opportunities.

**Highlights**
- User authentication
- Event management
- Volunteer matching
- Full-stack web architecture

---

## 💻 Technical Skills

### Languages
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat-square)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square)
![C++](https://img.shields.io/badge/C++-00599C?style=flat-square)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat-square)

### Frontend
![React](https://img.shields.io/badge/React-61DAFB?style=flat-square)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat-square)
![Tailwind CSS](https://img.shields.io/badge/Tailwind-06B6D4?style=flat-square)

### Backend & Infrastructure
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat-square)
![Django](https://img.shields.io/badge/Django-092E20?style=flat-square)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square)
![AWS](https://img.shields.io/badge/AWS-FF9900?style=flat-square)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square)

### AI & Data
![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=flat-square)
![Gemini](https://img.shields.io/badge/Gemini-4285F4?style=flat-square)
![RAG](https://img.shields.io/badge/RAG-000000?style=flat-square)
![Vector DBs](https://img.shields.io/badge/Vector_Databases-8B5CF6?style=flat-square)

---

## 🏆 Achievements

- 🥈 2nd Place — DubHacks
- 🏅 Best Use of AI with Reach Capital (MLH Prize)
- 🥈 2nd Place — Databricks
- 🧑‍🏫 Teaching Assistant for CS course, University of Washington 
- 👩‍💻 Resident Adviser, University of Washington

---

## 📚 Currently Exploring

- Machine Learning 
- Distributed Systems
- Retrieval-Augmented Generation (RAG)
- Cloud Architecture
- Product Engineering
- AI Applications in Education

---

## 📫 Connect

- LinkedIn: [Your LinkedIn](https://linkedin.com/in/mimilwang)
- Email: mimilhwang@gmail.com
- Portfolio: coming soon 🚀
